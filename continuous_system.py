...
class ContinuousSystem:
    ...
    def gen_nonlin_func(self, theta: np.ndarray):
        def nonlin_func(t: float, x: np.ndarray) -> np.ndarray:
            u = theta.reshape(4) @ x
            denom = self.m ** 2 * self.l ** 2 * np.cos(x[0]) ** 2 - (
                    self.m + self.M
            ) * (self.I + self.m * self.l ** 2)
    ...
    def lqr_control(self, q: list[float]):
        theta, _, _ = ct.lqr(self.A, self.B, np.diag(q), np.identity(1))
        return -theta

    def lin_sol(self, end_time: float, x0: np.ndarray, theta: np.ndarray):
        sol = solve_ivp(lambda t, x: (self.A + self.B @ theta) @ x, (0, end_time), x0, method="LSODA")
        u = sol.y.T @ theta.T
        return sol.t, sol.y, u

    def nonlin_sol(self, end_time: float, x0: np.ndarray, theta: np.ndarray):
        sol = solve_ivp(self.gen_nonlin_func(theta), (0, end_time), x0, method="LSODA")
        u = sol.y.T @ theta.T
        return sol.t, sol.y, u
